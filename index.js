...
function listOfNames(artistArray) {
  const artistArrayCopy = [...artistArray];
  for(let i = 0; i < artistArrayCopy.length; i++){
    const name = artistArrayCopy [i].name;
    artistArrayCopy[i] = name;
  }
  return artistArrayCopy;
}
listOfNames(artists);
...
function removeArtist(artistArray, number) {
  const artistArrayCopy = [...artistArray];
  artistArrayCopy.splice(number, 1);
  return artistArrayCopy;
}
...
function artistByCountry(artistArray, nationality){
  const artistByNationalityArray = []
  for(let i = 0; i < artistArray.length; i++){
    const artist = artistArray[i];
    if (artist.nationality === nationality){
      artistByNationalityArray.push(artist.name);
    }
  }
  return artistByNationalityArray;
}
